...
def find_similar_images(new_image_path: str, top_k: int = 5):
    ...
    try:
        ...
        # Display the results
        print(f"Top {top_k} most similar items:")
        for result in search_results:
            payload = result.payload
            similarity_score = result.score
            print(f"File Path: {payload['file_path']}, Description: {payload['description']}, "
                  f"Product ID: {payload['product_id']}, Similarity Score: {similarity_score}")
